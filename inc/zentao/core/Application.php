...
namespace zentao\core;

/**
 * 模拟zentao framework的基本环境 以实现重用zentao的核心功能代码
 */
class Application {
    ...
    public function setClientLang($lang = '') {
        if (!empty($lang)) {
            ...
            if (stripos($this->clientLang, 'hans'))
                $this->clientLang = 'zh-cn';
            if (stripos($this->clientLang, 'hant'))
                $this->clientLang = 'zh-tw';
        }
        if (!empty($this->clientLang)) {
            $this->clientLang = strtolower($this->clientLang);
            if (!isset($this->config->langs[$this->clientLang]))
    ...
    public function setClientTheme($theme = '') {
        ...
        if (!isset($_COOKIE['theme']))
            $_COOKIE['theme'] = $this->clientTheme;
    }
    ...
    public function parsePathInfo() {
        ...
        if (empty($pathInfo))
            $pathInfo = $this->getPathInfo('ORIG_PATH_INFO');
        if (!empty($pathInfo)) {
    ...
    public function getURI($full = false) {
        if ($full and $this->config->requestType == 'PATH_INFO') {
            if ($this->URI)
                return $this->config->webRoot . $this->URI . '.' . $this->viewType;
            return $this->config->webRoot;
        }
        return $this->URI;
    }
    ...
    public function setRouteByPathInfo() {
        if (!empty($this->URI)) {
            ...
            /* No reqeust seperator, use the default method name. */ else {
    ...
    public function setParamsByPathInfo($defaultParams = array()) {
        ...
        /* The full mode, both key and value passed in. */ elseif ($this->config->pathType == 'full') {
        ...
        if (!isset($this->config->strictParams) or $this->config->strictParams == false) {
            ...
            foreach ($defaultParams as $key => $defaultValue) {
                if (isset($passedParams[$i])) {
                    ...
                    if ($defaultValue === '_NOT_SET')
                        $this->triggerError("The param '$key' should pass value. ", __FILE__, __LINE__, $exit = true);
                }
                $i ++;
            }
        }
    ...
    public function loadConfig($moduleName, $exitIfNone = true) {
        ...
        if (!is_file($mainConfigFile)) {
            if ($exitIfNone)
                self::triggerError("config file $mainConfigFile not found", __FILE__, __LINE__, true);
            if (empty($extConfigFiles) and ! isset($config->system->$moduleName))
                return false;  //  and no extension file or extension in db, exit.
        ...
        foreach ($configFiles as $configFile) {
            if (in_array($configFile, $loadedConfigs))
                continue;
            include $configFile;
        ...
        foreach (array('system', 'personal') as $type) {
            if ($moduleName != 'common' and isset($config->$type->$moduleName)) {
                foreach ($config->$type->$moduleName as $item) {
                    if ($item->section) {
                        if (!isset($config->{$moduleName}->{$item->section}))
                            $config->{$moduleName}->{$item->section} = new stdclass();
    ...
    public function loadLang($moduleName) {
        ...
        if (!is_file($mainLangFile)) {
            if (empty($extLangFiles))
                return false;  // also no extension file.
    ...
    public function connectDB() {
        ...
        if (isset($config->db->host))
            $this->dbh = $dbh = $this->connectByPDO($config->db);
    ...
    public function saveError($level, $message, $file, $line) {
        ...
        if (strpos($message, 'Redefining') !== false)
            return true;
        ...
        if ($fh)
            fwrite($fh, strip_tags($errorLog)) && fclose($fh);
